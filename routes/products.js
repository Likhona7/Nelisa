...
exports.searchProducts = function(req, res, next) {
  req.getConnection(function(err, connection) {
    ...
    connection.query('select products.id, products.description as product_name, categories.description as category_name from products inner join categories on products.category_id = categories.id where products.description  like ? or categories.description like ?', [search_val, search_val], function(err, results) {
      if (err) return next(err);
